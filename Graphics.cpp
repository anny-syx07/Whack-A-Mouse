...
void Graphics::init() {
    ...
    if(TTF_Init()==-1)
    {
        logErrorAndExit("TTF could not intialize! %s\n",TTF_GetError());
    }
}
...
void Graphics::toggle(Mix_Music *gMusic)
{
...
Mix_Chunk* Graphics::loadChunk(const char* path)
{
    Mix_Chunk *gChunk=Mix_LoadWAV(path);
    if(gChunk==nullptr)
    {
        SDL_LogMessage(SDL_LOG_CATEGORY_APPLICATION,SDL_LOG_PRIORITY_ERROR,"Coulnd not load chunk: %s",Mix_GetError());
    }
    return gChunk;
}
